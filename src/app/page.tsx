...
import { FadeIn, TextReveal, ParallaxImage, StaggerContainer, StaggerItem, HoverCard } from "@/components/ui/Motion";
  ...
  return (
    <div className="w-full max-w-[1120px] mx-auto px-5 lg:px-8 py-16 md:py-[120px] flex flex-col gap-16 md:gap-[120px]">
      ...
      <section className="flex flex-col md:flex-row gap-8 md:gap-12 items-start md:items-center">
        <ParallaxImage className="w-32 h-32 md:w-48 md:h-48 flex-shrink-0 rounded-full overflow-hidden border border-stone/50 p-1">
        ...
        <div className="max-w-2xl">
          ...
          <FadeIn delay={0.4}>
            <p className="font-sans text-lg md:text-xl text-ink/80 leading-[1.7]">
              ...
              Somewhere between systems and stories.
            </p>
          </FadeIn>
        </div>
      </section>
          ...
          <StaggerContainer className="flex flex-col gap-6">
            {thoughts.map((thought) => (
              <StaggerItem key={thought.slug}>
                <Link href={`/thoughts/${thought.slug}`} className="group flex flex-col md:flex-row md:items-baseline gap-2 md:gap-8 p-4 -mx-4 rounded-lg hover:bg-surface transition-colors duration-300">
